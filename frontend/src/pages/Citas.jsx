...
export default function Citas({ idCliente }) {
  ...
  // 🟢 ESTADOS PARA EL MODAL DEL NUEVO VEHÍCULO
  ...
  // 🟢 FUNCIÓN: Cargar vehículos usando el ID real
  ...
  // 🟢 FUNCIÓN: Guardar el auto desde el Modal usando el ID real
  const handleGuardarAuto = async () => {
    ...
    try {
      const respuesta = await axios.post('http://localhost:3000/api/vehiculos', {
        ...nuevoAuto,
        id_cliente: idCliente // 🟢 Usamos el ID real
      });
  ...
  // 🟢 FUNCIÓN LIMPIA: Guardar la cita usando el ID real
  const handleAgendar = async (e) => {
    ...
    try {
      const respuesta = await axios.post('http://localhost:3000/api/citas', {
        id_cliente: idCliente, // 🟢 Usamos el ID real para la cita
        placa: placa,
